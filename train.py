...
def fit(model, criterion, optimizer, **kwargs):
    ...
    **kwargs : dict
    ...
    for i, data in enumerate(kwargs['dataloader']):
        ...
        if kwargs['lr_finder'] is True:
            optimizer = optim.Adam(kwargs['parameters'],
                                   lr=kwargs['lr_range_val'][i])
...
def train(train_loader_low_resolution, train_loader_full_resolution,
          val_loader_low_resolution, val_loader_full_resolution,
    ...
    for epoch in range(80):
        if 25 < epoch <= 40:
            if unfreeze_flag:
                ...
                unfreeze_flag = False
            train_loss, _, train_iou = fit(model, criterion, optimizer,
                                           parameters=params,
                                           dataloader=train_loader_low_resolution,
                                           phase='Training', lr_range_val=[],
                                           lr_finder=False, device=cuda0)
            train_loss_values.append(train_loss)
            val_loss, _, val_iou = fit(model, criterion, optimizer,
                                       parameters=params,
                                       dataloader=val_loader_low_resolution,
                                       phase='Validation', lr_range_val=[],
                                       lr_finder=False, device=cuda0)
            ...
            train_loss, _, train_iou = fit(model, criterion, optimizer,
                                           parameters=params,
                                           dataloader=train_loader_full_resolution,
                                           phase='Training', lr_range_val=[],
                                           lr_finder=False, device=cuda0)
            train_loss_values.append(train_loss)
            val_loss, _, val_iou = fit(model, criterion, optimizer,
                                       parameters=params,
                                       dataloader=val_loader_full_resolution,
                                       phase='Validation', lr_range_val=[],
                                       lr_finder=False, device=cuda0)
            val_loss_values.append(val_loss)
        else:
