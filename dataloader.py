...
class SoccerDataset(Dataset):
    """This is a class for creating an iterable dataset of images
    and annotated masks"""
    ...
    def __init__(self, path, image_dir, mask_dir, image_size, transform=None):
        """Constructor method for the class
            ...
            transform : string
                Name of transforms to be applied to the dataset.
                Eg: h_flip (horizontal flip)
    ...
    def __getitem__(self, idx):
        ...
        The function iterates through the images as per the index argument,
        reads them and resizes them according to the required image size,
        and returns a dictionary of image and annotated mask tensors.
        ...
        mask_path = os.path.join(self._path, self._mask_dir,
        ...
        mask = cv2.resize(mask, (self._image_size[0]//4,
                          self._image_size[1]//4))

        image, mask = self._get_transforms(image, mask, self._transform)

        image = np.transpose(image, [2, 0, 1]).astype(np.float32)

        img_mask_pair = {'image': torch.from_numpy(image),
    ...
    def _get_transforms(self, image, mask, transforms):
        ...
        Only horizontal flip transform which is required for this project
        is implemented. Additional transforms can be implemented by added
        several if else loops in the code below.
...
def get_dataloader(dataset, batchsize):
    ...
    It implements a train-validation split using SubsetRandomSampler
    in Pytorch. We use a train-val split of 80-20%. First random 80%
    of the images in the dataset are chosen and fed into the train_sampler.
    The remaining 20% are then put into val_sampler.
