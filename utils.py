...
class CyclicLearningRate:
    """Implementation based on Smith, Leslie N. "Cyclical learning rates
    ...
    def get_learning_rate(self):
        ...
        ax.set(title='LR plot', xlabel='iterations',
...
def plot_mask(prediction, label, cmap):
    ...
    for i in range(1, prediction.shape[0]):
        ...
        plt.savefig(os.path.join(image_path, 'label_{0}.png'.format(i)),
    ...
    for i in range(1, prediction.shape[0]):
        ...
        plt.savefig(os.path.join(image_path, 'predicted_mask_{0}.png'.format(i)),
                    bbox_inches='tight', pad_inches=0)
        plt.tight_layout()
...
def lr_finder_plot(lr_array, batch_loss):
    ...
    ax1.set(title='lr finder', xlabel='learning rate',
            ylabel='loss', xscale='log')
